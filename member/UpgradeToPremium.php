...
<?php 
//$mallu = 1480;
//echo "current balance".$mallu. " required balance = ".$lpayment;
if($mallu < $lpayment):
	echo " <h1>Sorry You don't have sufficient fund to upgrade your account.You need amount $".$lpayment." to upgrade your account.<br>";
else:
...
// comission to another user also 

$grandsponser = mysql_fetch_array(mysql_query("SELECT refid FROM users WHERE mid='".$sponsor[0]."'"));
$comAmount = $comisionAmount[0];
$currentBalanceOfSponsor = mysql_fetch_array(mysql_query("SELECT mallu FROM users WHERE mid='".$grandsponser[0]."'"));
$updatedBalanceOfSponsor = $currentBalanceOfSponsor[0]+$comAmount;
mysql_query("UPDATE users SET mallu='".$updatedBalanceOfSponsor."' WHERE mid='".$grandsponser[0]."'");
$des = "Referral Commision From $user";
////------------------------------>>>>>>>>>TRX
mysql_query("INSERT INTO trx SET usid='".$grandsponser[0]."', trx='".$txn_id."', tm='".time()."', description='".$des."', amount='".$comAmount."', nbal='".$updatedBalanceOfSponsor."'");
////------------------------------>>>>>>>>>TRX
////------------------------------>>>>>>>>>INCOME
mysql_query("INSERT INTO my_income SET usid='".$grandsponser[0]."', amount='".$comAmount."', description='".$des."', tm='".time()."', typ='R'");
/// end 
