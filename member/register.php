...
<div class="row">
		<div class="col-md-8 col-md-offset-2">
			...
			$nouser= false;	
			$free= false;		
			if(isset($_GET['checkrefer']) && !empty($_GET['checkrefer'])):
				$refname = $_GET['checkrefer'];
				// check user exist 
				$rrr = mysql_fetch_array(mysql_query("SELECT COUNT(*) FROM users WHERE username='".$refname."'"));
				// check paid user 
				$psts = mysql_fetch_array(mysql_query("SELECT paid_status FROM users WHERE username='".$refname."'"));
...
echo ($nouser) ? "<div class=\"alert alert-warning alert-dismissable\">Sorry! user doesn't exist with this ID.</div>" : '';
echo ($free) ? "<div class=\"alert alert-warning alert-dismissable\">Sorry! you can't register yourself under this user becuause this user is not premium user.</div>" : '';
			endif;
			...
			<form action="" method="get">
					<section>
		<label class="input">Enter Refer Id here.</label></section>
		<input type="number" name="checkrefer" placeholder="Referrer ID" id="refname" class="form-control"> <br/>
		<button class="btn-u btn-u-sea-shop btn-block margin-bottom-20" type="submit">Submit</button>
...
$cheking = checknoofnode($refid[0], $position);
$position= $cheking['position'];
if($cheking['position']==false && $cheking['position']==false){
$err21=1;	
}
$willPosition = getLastChildOfLR($refname,$position);
$pos = mysql_fetch_array(mysql_query("SELECT COUNT(*) FROM users WHERE posid='".$willPosition."' AND position='".$position."' AND refid='".$refid[0]."'"));
...
Registretion Completed Successfully and your login id is ".$_POST['username']."</div>";
...
echo "<div class=\"alert alert-danger alert-dismissable\">
<button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button>  
This member doesn't have more space for you.
</div>";
}   
}
}
?>
		...
		<section>
		<label class="input">
<input type="text" name="refname" placeholder="Referrer ID" id="refname" class="form-control" readonly>
		</label>
		</section>
		</div>
		<div class="col-sm-6">
...
<div class="col-xs-12">
    ...
    <option value="Zimbabwe">Zimbabwe</option>
		...
		<section>
		<label class="input">
		<input type="hidden" name="username" placeholder="Jyonty" id="username" class="form-control" value="<?php echo rand(); ?>">
		</label>
		</section>
