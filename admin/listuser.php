...
<?php
$llevel= array('Star', 'Emerald', 'Rubby', 'Amethyst', 'Diamond', 'Red Diamond');
...
<?php
$ddaa = mysql_query("SELECT mid, refid, posid, position, username, mallu, level FROM users WHERE email='".$email."'");
echo mysql_error();
...
<td><b class='btn btn-danger btn-xs'>$llevel[$lllev]</b></td>
...
<?php
$ddaa = mysql_query("SELECT mid, refid, posid, position, username, mallu, level FROM users WHERE mobile='".$number."'");
echo mysql_error();
while ($data = mysql_fetch_array($ddaa)){
$refBy = mysql_fetch_array(mysql_query("SELECT username FROM users WHERE mid='".$data[1]."'"));
$posUnder = mysql_fetch_array(mysql_query("SELECT username FROM users WHERE mid='".$data[2]."'"));
...
<?php
$ddaa = mysql_query("SELECT mid, refid, posid, position, username, mallu, level FROM users WHERE username='".$uuid."'");
echo mysql_error();
while ($data = mysql_fetch_array($ddaa)){
$refBy = mysql_fetch_array(mysql_query("SELECT username FROM users WHERE mid='".$data[1]."'"));
$posUnder = mysql_fetch_array(mysql_query("SELECT username FROM users WHERE mid='".$data[2]."'"));
...
<?php
$ddaa = mysql_query("SELECT mid, refid, posid, position, username, mallu, level FROM users ORDER BY mid DESC LIMIT ".$start.",30");
echo mysql_error();
while ($data = mysql_fetch_array($ddaa)){
$refBy = mysql_fetch_array(mysql_query("SELECT username FROM users WHERE mid='".$data[1]."'"));
$posUnder = mysql_fetch_array(mysql_query("SELECT username FROM users WHERE mid='".$data[2]."'"));
