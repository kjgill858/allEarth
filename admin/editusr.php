...
$res = mysql_query("UPDATE users SET  fname='".$firstname."', lname='".$lastname."', dob='".$dob."', address='".$address."', city='".$city."', postcode='".$post."', country='".$country."', mobile='".$mobile."', email='".$email."', level='".$level."' WHERE mid='".$iidd."'");
if($res){
...
$old = mysql_fetch_array(mysql_query("SELECT mid, refid, posid, position, username, password, joindate, fname, lname, dob, address, city, postcode, country, mobile, email, status, paid_status, level FROM users WHERE mid='".$iidd."'"));
...
<div class="form-group">
<label class="col-md-3 control-label"><strong>AccountType</strong></label>
<div class="col-md-6">
	<?php //var_dump($old['level']); ?>
	<select  name="level" class="form-control input-lg">
		<option value="1" <?php echo ($old['level']==1)? 'selected' : '';?>>Star</option>
		<option value="2" <?php echo ($old['level']==2)? 'selected' : '';?>>Emerald</option>
		<option value="3" <?php echo ($old['level']==3)? 'selected' : '';?>>Rubby</option>
		<option value="4" <?php echo ($old['level']==4)? 'selected' : '';?>>Amethyst</option>
		<option value="5" <?php echo ($old['level']==5)? 'selected' : '';?>>Diamond</option>
		<option value="6" <?php echo ($old['level']==6)? 'selected' : '';?>>Red Diamond</option>
	</select>
